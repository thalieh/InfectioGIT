...
def get_all_models(query, path):
    ...
    while True:
        try:
            ...
            print(f"Erreur : {e}")
            break
    res=download_and_enrich(models, path)
...
def download_and_enrich(model_data, base_directory):
    ...
    try:
        ...
        directory = os.path.join(base_directory, sub_folder) # on recupere la direction fournis par l'utilisateur et standardisation
        os.makedirs(directory, exist_ok=True) # création du dossier de l'infection si il existe pas déjà
    ...
    except Exception as e: # gérer exception si programme plante
        print(f"❌ Erreur sur {model_id}: {e}")

def main():
    # 2. LA QUERY PAR ONTOLOGIE
    # On cherche les modèles qui ont ces codes dans leurs métadonnées
    # Exemple : MONDO_0005066 (COVID), MONDO_0005109 (HIV), etc.
    mondo_ids = ["MONDO_0005109", "MONDO_0005066", "MONDO_0018076", "MONDO_0005136", "MONDO_0005131"] # notre liste de codes
    ...
    base_dir = "Infectio_Ontology_Data" # pour bien organiser donnees dans github
    models = get_all_models(query) # appelle de la recherche

    for m in models: # enrichissemnt de tous les modèles trouvés
